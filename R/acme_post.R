...
#'@param xl 2-element vector of plotting ranges. Default first element of 0, 
...
#'  
#'@return The function returns a vector with input C, ACME estimate, posterior
...
###################################################################
ObjPost <- function(C=0, Rstar=0.2496, T=0.1740, gam=c(0.5, 0.9), I=7,
  ...
  if(missing(xl)) xl <- c(0, M[Mtop]) else Mtop <- xl[2];
#
  if(nchar(ps)>0)
...
# Plotting
  ...
  legend(1.01*xl[2], 1.10*max(pmf), xjust=1, yjust=1,
    c(expression(""),
    bquote(C[i] == .(C)),
